...
public class RaceController {
	...
	@ResponseBody
	public AllInfo addRaInfo(@RequestParam("usId") Integer usId,
			                @RequestParam("raName") String raName,
		                   	@RequestParam("raCategory") String raCategory,
		                   	@RequestParam("Cdate") String Cdate,
		                   	@RequestParam("raLevel") String raLevel,
		                   	@RequestParam("raType") String raType,
		                   	@RequestParam("raTeacher") String raTeacher,
		                   	@RequestParam("raAbout") String raAbout) throws ParseException{
		
		logger.info("addRaInfo++"+usId+"++"+raName+"++"+raCategory+"++"+Cdate+"++"+raLevel+"++"+raType+"++"+raTeacher+"++"+raAbout);
		int re = this.raceservice.addRaInfo(new Race(usId,raName,raCategory,raType,
				raceservice.getUserNameById(usId),raLevel,raTeacher,(Date)(new SimpleDateFormat("yyyy-MM-dd").parse(Cdate)),raAbout,new Date(System.currentTimeMillis())));
		return  new AllInfo(String.valueOf(re));
	}
	
	@RequestMapping(value="/upRaInfo" , method = RequestMethod.GET)
	@ResponseBody
	public AllInfo upRaInfo(@RequestParam("usId") Integer usId,
		...
		logger.info("upRaInfo++"+usId+"++"+raId+"++"+raName+"++"+raCategory+"++"+Cdate+"++"+raLevel+"++"+raType+"++"+raTeacher+"++"+raAbout);
		int re = this.raceservice.upRaInfo(new Race(raId,usId,raName,raCategory,raType,
				raceservice.getUserNameById(usId),raLevel,raTeacher,(Date)(new SimpleDateFormat("yyyy-MM-dd").parse(Cdate)),raAbout,new Date(System.currentTimeMillis())));
		return  new AllInfo(String.valueOf(re));
	}
}
